    ...
    <div class="container">
        <header
            ...
            <ul class="nav nav-pills col-12 col-md-auto mb-3 justify-content-center mb-md-0">
                ...
                <!--        <router-link to="./FAQs" class="nav-item nav-link" :class="{'active': location.pathname === '/FAQs'}">FAQs</router-link>-->
                <!--        <router-link to="./About" class="nav-item nav-link" :class="{'active': location.pathname === '/About'}">About</router-link>-->
            </ul>

            <div class="col-md-3 text-end">
                <div @click="setLoginViewEvent('login')" class="btn btn-outline-primary me-2">Login</div>
                <div @click="setLoginViewEvent('signup')" class="btn btn-primary">Sign-up</div>
            </div>
        </header>
    </div>
    <!--    <LoginView :showMode="loginViewEvent" @eventListener="loginViewEventListener"/>-->
    <LoginView ref="LoginViewRef"/>
</template>
...
const activeNavNum = ref(0);
const loginViewEvent = ref("hide");
const LoginViewRef = ref();

function setLoginViewEvent(event) {
    console.log(`Preset: ${event}`)
    // loginViewEvent.value = event;
    switch (event) {
        case "login":
            LoginViewRef.value.actionView("login");
            break;
        case "signup":
            LoginViewRef.value.actionView("signup");
            break;
        case "hide":
            LoginViewRef.value.actionView("hide");
            // loginViewEvent.value = "hide";
            break;
    }
}

function loginViewEventListener(event) {
